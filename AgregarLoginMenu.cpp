...
// Calcular promedio de temperatura por un mes 
...
// Función para mostrar el menú de datos meteorológicos
...
int main() {
    ...
    if (archivoUsuarios) {
        while (archivoUsuarios.good()) {
            Persona usr;
            archivoUsuarios.read(reinterpret_cast<char*>(&usr), sizeof(Persona));
    ...
    if (leerDatosMeteorologicos("datos_meteorologicos.txt", datos, numDatos, anioMin, mesMin, diaMin, anioMax, mesMax, diaMax) != 0) {
        return 1;
    }

    cout << "Datos meteorológicos cargados correctamente." << endl;
    cout << "Fecha mínima disponible: " << anioMin << "-" << setw(2) << setfill('0') << mesMin << "-" << setw(2) << setfill('0') << diaMin << endl;
    cout << "Fecha máxima disponible: " << anioMax << "-" << setw(2) << setfill('0') << mesMax << "-" << setw(2) << setfill('0') << diaMax << endl;
