...
void encontrarMaximaMinimaTemperatura(const DatosMeteorologicos datos[], int numDatos) {
    // Llamar a encontarRangos para obtener los rangos de fechas
    encontarRangos(datos, numDatos);

    // Variables para almacenar las temperaturas máxima y mínima
    double temperaturaMaxima = -999; // Inicializado con un valor muy bajo
    double temperaturaMinima = 999; // Inicializado con un valor muy alto
    ...
    if (!encontrado) {
        cout << "No se encontraron datos para el rango de fechas ingresado."
             << endl;
        return;
    }

    cout << "Temperatura máxima en el rango de fechas: " << temperaturaMaxima
        << " C" << endl;
    cout << "Temperatura mínima en el rango de fechas: " << temperaturaMinima
        << " C" << endl;
}
 

// Función para calcular el promedio de temperatura para un mes específico
void calcularPromedioTemperaturaPorMes(DatosMeteorologicos datos[],
                                       int numDatos) {
  int mes;
  cout << "Ingrese el número de mes para calcular el promedio de "
          "temperatura: ";
  cin >> mes;
...
void calcularMayorPromedioHumedad(DatosMeteorologicos datos[], int numDatos) {
  string condicion;
  cout << "Ingrese la condición meteorológica para calcular el promedio de "
          "humedad: ";
  cin >> condicion;
  ...
  if (!encontrado) {
    cout << "No hay datos para la condición meteorológica especificada."
         << endl;
    return;
  }
...
void calcularMenorPromedioHumedad(DatosMeteorologicos datos[], int numDatos) {
  ...
  // Calcular el menor promedio de humedad para la condición meteorológica
  // especificada
  for (int i = 0; i < numDatos; ++i) {
    if (datos[i].condicionMeteorologica == condicion) {
      totalHumedad += datos[i].humedad;
  ...
  cout << "El menor promedio de humedad para la condición " << condicion
       << " es: " << fixed << setprecision(2) << menorPromedio << " %" << endl;
}

// Función para mostrar datos meteorológicos en un rango de fechas
void mostrarDatosMeteorologicosPorRango(DatosMeteorologicos datos[],
                                        int numDatos) {
  int anio1, mes1, dia1;
  int anio2, mes2, dia2;
...
int main() {
  ...
  while (true) {
    // Menú
    cout << "\n--- Menú ---" << endl;
    cout << "1. Encontrar temperatura máxima y mínima en un rango de fechas"
         << endl;
    cout << "2. Calcular promedio de temperatura para un mes específico"
         << endl;
    cout << "3. Calcular el mayor promedio de humedad en relación a una "
            "condición meteorológica"
         << endl;
    cout << "4. Calcular el menor promedio de humedad en relación a una "
            "condición meteorológica"
         << endl;
    ...
    default:
      cout << "Opción no válida. Por favor, ingrese un número entre 1 y 7."
           << endl;
    }
  }
